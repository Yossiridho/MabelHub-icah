const rawData = `Kabupaten Aceh Barat	Aceh
...
Kabupaten Kep. Seribu	Dki Jakarta
...
Kabupaten Bolaang Mongondowselatan	Sulawesi Utara
Kabupaten Bolaang Mongondowtimur	Sulawesi Utara
Kabupaten Bolaang Mongondowutara	Sulawesi Utara
Kabupaten Kep. Siau Tagulandangbiaro	Sulawesi Utara
...
Kabupaten Ogan Komering Uluselatan	Sumatera Selatan
Kabupaten Ogan Komering Ulutimur	Sumatera Selatan
Kabupaten Penukal Abab Lematangilir	Sumatera Selatan
...
export const listKabupatenKota: Wilayah[] = rawData
    .trim()
    .split('\n')
    .map(line => {
        const [nama, provinsi] = line.split('\t')
        return { nama: nama.trim(), provinsi: provinsi.trim() }
    })

// Provinsi di-derive otomatis — tidak perlu tulis manual, tidak bisa duplikat
export const listProvinsi = [
    { value: '', label: '-- Pilih Provinsi --' },
    ...Array.from(new Set(listKabupatenKota.map(w => w.provinsi)))
        .sort()
